...
```bash
# Option 1 — curl (recommended, no git clone required)
curl -fsSL https://raw.githubusercontent.com/leqtrungit/stratum/main/bootstrap.sh | bash

# Option 2 — git clone
git clone https://github.com/leqtrungit/stratum.git my-project
...
> **Requirements:** `docker`, `docker compose`, `git`, and `bash`. No Node.js or other runtimes required.
...
```
stratum/
├── bootstrap.sh                # curl | bash entry point — downloads template and runs install.sh
├── install.sh                  # Interactive setup script (run inside a cloned/bootstrapped project)
